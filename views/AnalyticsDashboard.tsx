...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ 
  ...
  const [customPrompt, setCustomPrompt] = useState<string>(''); // For custom KPI requests
  const [customAnalysis, setCustomAnalysis] = useState<string>(''); // For custom analysis results
  const [isAnalyzingPrompt, setIsAnalyzingPrompt] = useState(false); // For loading state
  ...
  const generateAutomaticKPIs = (rows: any[], headers: string[]) => {
    ...
    // Always update the data state with automatic KPIs if no meaningful KPIs were generated by AI
    if (data && (data.kpis.length === 0 || (data.kpis.length === 1 && data.kpis[0].label === "Total Records"))) {
      setData({
        ...data,
    ...
    // If no data at all, set the automatic KPIs
    if (!data) {
      setData({
        kpis: kpis.slice(0, 4),
        dailySummary: "No data available",
        weeklySummary: "No data available",
        monthlySummary: "No data available",
  ...
  const handleCustomPrompt = async () => {
    if (!customPrompt.trim() || !data) return;
    ...
    try {
      ...
      if (parsedRows.length > 0) {
        // Create a CSV sample of the first 10 rows
        dataSample += headers.join(",") + "\n";
        const sampleRows = parsedRows.slice(0, Math.min(10, parsedRows.length));
        sampleRows.forEach(row => {
          dataSample += headers.map(h => `"${row[h] || ''}"`).join(",") + "\n";
        });
      }
      
      // Create a prompt for the AI to generate KPIs or analysis
      const fullPrompt = `Based on the following dataset, please ${customPrompt}.
      ...
      Please provide a detailed response with relevant metrics, KPIs, or analysis as requested.`;
      
      // Use the existing analyzeSalesData function for consistency
      const report = await analyzeSalesData(fullPrompt);
      
      // Format the response for display
      let response = "";
      if (report.dailySummary && report.dailySummary !== "No data available") {
        response += report.dailySummary + "\n\n";
      }
      if (report.kpis && report.kpis.length > 0) {
        response += "Generated KPIs:\n";
        report.kpis.forEach(kpi => {
          response += `- ${kpi.label}: ${kpi.value} (${kpi.change})\n`;
        });
      }
      ...
      setCustomAnalysis(response || "No specific analysis was generated for your request.");
    } catch (error) {
      console.error("Custom prompt analysis error:", error);
      setCustomAnalysis("Sorry, I encountered an error while processing your request. Please try again.");
    } finally {
